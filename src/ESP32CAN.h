...
class ESP32CAN {
    public:
        ESP32CAN_status_t initCan();
        ESP32CAN_status_t initCan(ESP32CAN_timing_t busSpeed);
        ESP32CAN_status_t initCAN(gpio_num_t tx_pin, gpio_num_t rx_pin, ESP32CAN_timing_t speed);
        ESP32CAN_status_t initCAN(gpio_num_t tx_pin, gpio_num_t rx_pin, ESP32CAN_timing_t speed, twai_mode_t MODE);
        ESP32CAN_status_t stopCAN();
        ESP32CAN_status_t sendCANMsg();
        ESP32CAN_status_t readCANMsg(uint32_t* msg_id, uint8_t* can_flags, uint8_t* data_length, uint8_t* data_bytes);
        ESP32CAN_status_t readCANMsg(twai_message_t* rx_msg);
    ...
    private:
        /* Default setup TX on GPIO5, RX on GPIO4, Normal Mode, accept all data frames (no filter configured), 
             Bus speed set to 250Kbps, ESP-IDF RX & TX queues both set to 5. */
        ...
        SemaphoreHandle_t tx_msg_lock = xSemaphoreCreateBinary();
        SemaphoreHandle_t rx_msg_lock = xSemaphoreCreateBinary();
